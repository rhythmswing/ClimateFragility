...
\begin{figure}[t]
    \centering
   \includegraphics[width=.8\linewidth]{figs/model.eps} 
...
\vpara{Environmental Performance Index (EPI).} It is an index to evaluate a state's environmental performance developed by Yale University~\cite{EPI_index}. It is composed of indicators in ecosystem vitality and environmental health. 

\vpara{Fragile States Index (FSI).} It is an index to measure a country's vulnerability to conflict developed by Fund for Peace and Foreign Policy by accounting for 12 social, economical, and political indicators.~\cite{FSI_index}

The composition of the two indexes are illustrated in Figure~\ref{fig:model:indexes}. 
...
Sometimes, we use binarized values of EPI and FSI indexes to denote the status of a state.
